...
//		[Glade.Widget]Gtk.ImageMenuItem QuitMenuItem;
		...
		public void calculateSHCoefficients()
		{
			...
			if (inputTempScale == KELVIN) {
				lT = lowTemp;
				mT = midTemp;
				hT = highTemp;
			} else if (inputTempScale == CELCIUS) {
...
//					}
					...
					if (outputTempScale == CELCIUS) {
						temp = temp - KelvinOffset;
					} else if (outputTempScale == FAHRENHEIT) {
						temp = (((temp - KelvinOffset) * 9) / 5) + FahrenheitOffset; 
					} else if (outputTempScale == KELVIN) {
